...
export function BranchDetailModal({ isOpen, onClose, branch }: Props) {
    ...
    const stats = [
        { label: 'Total Staff', value: branch.staffs?.length || 0, icon: Users2, color: 'text-blue-600', bg: 'bg-blue-50' },
        { label: 'Total Customers', value: branch.customers?.length || 0, icon: Users, color: 'text-blue-600', bg: 'bg-blue-50' },
        { label: 'Active Loans', value: branch.loans?.filter(l => l.status.toLowerCase() === 'active').length || 0, icon: Activity, color: 'text-green-600', bg: 'bg-green-50' },
        { label: 'Avg Portfolio', value: '0.0M', icon: TrendingUp, color: 'text-amber-600', bg: 'bg-amber-50' },
    ];

    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-300">
            <div className="bg-white rounded-3xl max-w-5xl w-full h-[85vh] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in duration-200">
                {/* Header Section */}
                <div className="relative p-8 bg-gradient-to-br from-blue-600 to-blue-800">
                    <button
                        onClick={onClose}
                    ...
                    {activeTab === 'overview' && (
                        <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
                            ...
                            <div className="grid grid-cols-4 gap-6">
                                {stats.map((stat, idx) => (
                                    <div key={idx} className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex items-center gap-5 group hover:shadow-md transition-all">
                            ...
                            <div className="grid grid-cols-3 gap-8">
                                ...
                                <div className="col-span-2 space-y-6">
                                    <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                                        <div className="flex items-center gap-3 mb-8">
                                            ...
                                            <h3 className="text-sm font-black text-gray-900 uppercase tracking-widest">Branch Information</h3>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-y-8 gap-x-12">
                                            {[
                                                ...
                                                <div key={i} className="flex gap-4">
                                                    <div className="p-2.5 bg-gray-50 rounded-xl h-fit">
                                ...
                                <div className="space-y-6">
                                    <div className="bg-blue-600 p-8 rounded-3xl text-white shadow-xl shadow-blue-100">
                                        ...
                                        <button className="w-full py-4 bg-white text-blue-600 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg active:scale-95 transition-all">
                                            View Profile
                                        </button>
                                    </div>
                                </div>
                                        ...
                                        <tr key={staff.staff_id} className="hover:bg-gray-50 transition-colors">
                                            <td className="px-8 py-4">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-10 h-10 bg-blue-100 text-blue-700 rounded-xl flex items-center justify-center font-black text-sm capitalize">
                                                        {staff.full_name[0]}
                                                    </div>
                                                    <div>
                                                        <p className="font-black text-gray-900 text-sm">{staff.full_name}</p>
                                                        <p className="text-[10px] font-bold text-gray-400 font-mono uppercase tracking-tighter">{staff.staff_id}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-8 py-4">
                                            ...
                                            <td className="px-8 py-4 text-right">
                                                <button className="p-2 hover:bg-gray-100 rounded-lg transition-all text-indigo-600">
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="bg-gray-50/50 border-b border-gray-100">
                                        ...
                                        <th className="px-8 py-5 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">NIC</th>
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {branch.customers?.map((customer: Customer) => (
                                        <tr key={customer.id} className="hover:bg-gray-50 transition-colors">
                                            <td className="px-8 py-4">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-10 h-10 bg-indigo-100 text-indigo-700 rounded-xl flex items-center justify-center font-black text-sm capitalize">
                                                        {customer.full_name[0]}
                                                    </div>
                                                    <div>
                                                        <p className="font-black text-gray-900 text-sm">{customer.full_name}</p>
                                                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">{customer.mobile_no_1}</p>
                                                    </div>
                                                </div>
                                            </td>
                    ...
                    {activeTab === 'loans' && (
                        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden animate-in slide-in-from-bottom-4 duration-500">
                            <table className="w-full">
                                <thead>
                                    <tr className="bg-gray-50/50 border-b border-gray-100">
                                        <th className="px-8 py-5 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Loan ID</th>
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {branch.loans?.map((loan: Loan) => (
                                        <tr key={loan.id} className="hover:bg-gray-50 transition-colors">
                                            <td className="px-8 py-4">
                                                <span className="font-black text-gray-900 text-sm">{loan.loan_id}</span>
                                            </td>
                                            <td className="px-8 py-4">
                                                <p className="font-black text-gray-900 text-sm">LKR {Number(loan.approved_amount).toLocaleString()}</p>
                                            </td>
                                            <td className="px-8 py-4">
                                                <span className={`px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider ${loan.status.toString().toLowerCase() === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'
                                                    }`}>
                                                    {loan.status}
                                                </span>
                                            </td>
