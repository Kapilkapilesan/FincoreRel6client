    ...
    return (
        <div className="w-full">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
                ...
                <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-900/50 p-1.5 rounded-2xl border border-gray-100 dark:border-gray-800">
                    <Filter className="w-3.5 h-3.5 text-gray-400 ml-2" />
                    <select
                        value={selectedBranchId || 'all'}
                        onChange={(e) => handleBranchSelect(e.target.value)}
                        className="px-4 py-2 text-sm font-bold border-none rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white shadow-sm ring-1 ring-gray-200 dark:ring-gray-700 focus:ring-2 focus:ring-indigo-500 transition-all outline-none"
                    >
            ...
            <div className="relative h-72 w-full group">
                {/* Y-Axis Labels */}
                <div className="absolute left-0 top-0 bottom-8 flex flex-col justify-between text-[10px] font-black text-gray-400 w-10 text-right pr-3">
                ...
                <div className="absolute left-10 right-0 top-0 bottom-8 flex flex-col justify-between pointer-events-none">
                    {[1, 0.75, 0.5, 0.25, 0].map((tick) => (
                        <div key={tick} className="w-full h-px bg-gray-100 dark:bg-gray-800/50" />
                    ))}
                </div>
                ...
                <div className="absolute left-10 right-0 bottom-0 flex justify-between px-2">
                    {data.map((item, index) => (
                        <span
                            key={item.month}
                            className={`flex-1 text-center text-[9px] font-black text-gray-400 uppercase tracking-tighter mt-3 transform transition-all 
