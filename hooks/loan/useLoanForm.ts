...
export const useLoanForm = () => {
        ...
        setFormData((prev) => {
            ...
            if (field === 'loanProduct') {
                ...
                if (product) {
                    ...
                    // Trigger fee calculation after product defaults
                    const t = parseInt(newData.tenure);
                    if (t === 48) newData.processingFee = (Number(newData.loanAmount) * 0.04).toFixed(2);
                    else if (t === 72) newData.processingFee = (Number(newData.loanAmount) * 0.06).toFixed(2);
                }
            }
    ...
    const loadFromLoan = useCallback((loan: Loan) => {
        setFormData({
            ...
            monthly_income: (loan as any).monthly_income?.toString() || '0',
            monthly_expenses: (loan as any).monthly_expenses?.toString() || '0',
            documents: {},
